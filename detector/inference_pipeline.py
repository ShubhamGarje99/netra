...
import os
import numpy as np
from incident_detector import VehicleCollisionDetector, CrowdGatheringDetector, FireDetector
...
class CameraPipeline:
    ...
    """
    def __init__(self, camera_id: str, config: dict, model, 
                 frame_buffer: dict, frame_lock: threading.Lock,
        ...
        # Detectors
        self.collision_detector = VehicleCollisionDetector()
    ...
    def start(self):
        ...
        print(f"[{self.camera_id}] Opened: {source} @ {fps:.1f}fps, seeking to frame {offset_frames}")
        
        grabber = threading.Thread(target=self._frame_grabber_thread, 
                                   args=(cap,), daemon=True,
                                   name=f"grabber-{self.camera_id}")
        inference = threading.Thread(target=self._inference_thread,
                                     daemon=True,
    ...
    def stop(self):
        self.running = False
        print(f"[{self.camera_id}] Pipeline stopped after {self.frames_processed} frames")
    ...
    def _frame_grabber_thread(self, cap):
        ...
        while self.running:
            ...
            with self.latest_frame_lock:
                self.latest_frame = frame  # overwrite — old frame is intentionally dropped
    ...
    def _inference_thread(self):
        ...
        while self.running:
            ...
            # Add overlay
            cv2.putText(annotated, f"{self.camera_id} · {self.config['name']}", 
                       (8, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 178), 1)
            cv2.putText(annotated, time.strftime("%H:%M:%S"), 
                       (8, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 178), 1)
            ...
            collision = self.collision_detector.analyze(detections, frame.shape)
            if collision: incidents.append(collision)
            
            crowd = self.crowd_detector.analyze(detections, frame.shape)
            if crowd: incidents.append(crowd)
            
            fire = self.fire_detector.analyze(frame, detections)
            if fire: incidents.append(fire)
            ...
            if self.frames_processed % 100 == 0:
                ...
                print(f"[{self.camera_id}] frame={self.frames_processed} "
